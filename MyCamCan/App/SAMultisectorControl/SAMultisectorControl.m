...
- (BOOL) beginTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event{
    ...
    for(NSUInteger i = 0; i < sectorsArray.count; i++){
        ...
        if([self touchInCircleWithPoint:touchPoint circleCenter:drawInf.startMarkerCenter]){
            trackingSector = sector;
            trackingSectorDrawInf = drawInf;
            _trackingSectorStartMarker = YES;
            return YES;
        }
...
- (void)drawSector:(SAMultisectorSector *)sector atPosition:(NSUInteger)position{
    ...
    CGContextAddArc(context, drawInf.startMarkerCenter.x, drawInf.startMarkerCenter.y, drawInf.startMarkerRadius-2.0, 0.0, 6.28, 0);
    ...
    CGContextAddArc(context, drawInf.endMarkerCenter.x, drawInf.endMarkerCenter.y, drawInf.endMarkerRadius-2.0, 0.0, 6.28, 0);
    CGContextSetFillColorWithColor(context, sector.color.CGColor);
    CGContextFillPath(context);
    
    // drawing start marker's text
    [self drawString:self.startText
            withFont:[UIFont systemFontOfSize:13 weight:UIFontWeightSemibold]
...
//    CGFloat minMarkerRadius = self.minCircleMarkerRadius / 4.0;
//    CGFloat maxMarkerRadius = self.maxCircleMarkerRadius / 2.0;
    
    drawInf.startMarkerRadius = 15.0;//self.minCircleMarkerRadius;//((drawInf.circleOffsetAngle/(self.startAngle + 2*M_PI)) * (maxMarkerRadius - minMarkerRadius)) + minMarkerRadius;
    drawInf.endMarkerRadius = 15.0;//self.maxCircleMarkerRadius;//((drawInf.circleLineAngle/(self.startAngle + 2*M_PI)) * (maxMarkerRadius - minMarkerRadius)) + minMarkerRadius;
