...
+(void)facebookVideoStage2:(NSData*)videoData comment:(NSString*)comment upload_session_id:(NSString *)upload_session_id account:(ACAccount*)account withCompletion:(VideoUploadCompletion)completion{
    ...
    if (account.credential.oauthToken == nil || upload_session_id == nil) {
        NSLog(@"Facebook Oauth Token: %@", account.credential.oauthToken);
        NSLog(@"upload_session_id: %@", upload_session_id);

        [SocialVideoHelper uploadError:[AppData errorWithCode:501 description:@"Facebook Oauth token nil or upload_session_id nil"]withCompletion:completion];
        return;
    }
